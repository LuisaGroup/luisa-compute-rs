...
fn main() {
    ...
    stream.with_scope(|s| {
        ...
        for (index, model) in models.iter().enumerate() {
            ...
            let m: Mat4 = unsafe { std::mem::transmute(m) };
            accel.push_mesh(&mesh, m, u32::MAX, false);
        }
    ...
    let path_tracer = Kernel::<fn(Tex2d<Float4>, Tex2d<u32>, Accel, Uint2)>::new_async(
        ...
        track!(|image: Tex2dVar<Float4>,
            ...
            let pixel = (coord.cast_f32() + Float2::expr(rx, ry)) / frame_size * 2.0f32 - 1.0f32;
            ...
            for _ in 0..SPP_PER_DISPATCH as u32 {
                ...
                let filter = |c: &TriangleCandidate| {
                    ...
                    if c.inst == 6u32 {
                        *valid = (c.bary.y * 5.0f32).fract() < 0.5f32;
                    }
                    **valid
                };
