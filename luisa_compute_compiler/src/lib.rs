        ...
        let  proxy_def = quote_spanned!(span=>
            ...
            impl #crate_path ::VarProxy<#name> for #proxy_name {
                fn from_node(node: #crate_path ::NodeRef) -> Self {
                    Self {
                        node,
                        son: #crate_path ::StructOfNodes::from_nodes(&mut std::iter::once(node)),
            ...
            span=>  
                ...
                impl #crate_path ::Value for #name {
                    ...
                    fn destruct(v: Var<Self>) -> Self::SoN {
                       ...
                       #(
                        let #field_names = {
                            let node = #crate_path ::__extract::<#field_types>(v.node(), &mut index);
                            <#field_types as #crate_path ::Value>::destruct(Var::<#field_types>::from_node(node))
                        };
                       )*
                       Self::SoN{
                           #( #field_names ),*
                    ...
                    fn construct(s: Self::SoN) -> Var<Self> {
                        ...
                        #(
                            nodes.append(
                                <#field_types as #crate_path ::Value>::construct(s.#field_names).node()
                            );
                        )*
                        let node = __compose::<Self>(&nodes);
                        Var::from_node(node)
                    }
                }
